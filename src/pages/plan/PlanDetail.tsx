...
import { Wifi, Shield, Gift, Smartphone, Star, MapPin, Loader2, Check, X } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { PlanResponse } from '@/types/plans';
import { Plan } from '@/types/plan';
import { cn } from '@/lib/utils';
import { changePlan, disconnectPlan } from '@/apis/plan/postPlan';
  ...
  const handlePlanDisconnect = async () => {
    ...
    if (!isLoggedIn) {
      openModal({
        id: 'login-required-modal',
        title: '로그인이 필요합니다!',
        description: '요금제 해지를 위해서는 로그인이 필요합니다.\n로그인 후 이용해주세요.',
        variant: 'default',
        size: 'sm',
        showClose: false,
        showCancel: false,
    ...
    openModal({
      id: 'plan-disconnect-confirm-modal',
      title: '요금제 해지 확인',
      description: `${plan.name} 요금제를 해지하시겠습니까?\n 해지 후에는 서비스 이용이 중단됩니다.`,
      variant: 'default',
      size: 'sm',
      showClose: false, // X 버튼은 숨깁니다
      showCancel: true, // 취소 버튼을 표시합니다
      showConfirm: true, // 확인 버튼을 표시합니다
      cancelText: '취소', // 취소 버튼 텍스트
      confirmText: '해지하기', // 해지 버튼 텍스트
      confirmVariant: 'destructive', // 해지 버튼에 destructive 스타일 적용
      closeOnOverlayClick: false, // 오버레이 클릭 시 모달 닫히지 않게 설정
      closeOnEscape: false, // ESC 키로 모달을 닫지 않게 설정
      onConfirm: async () => {
        await executePlanDisconnect(); // 해지하는 작업 실행
      },
    });
  };

  const executePlanDisconnect = async () => {
    if (!plan || !id) return;

    setIsDisconnecting(true);

    try {
      const result = await disconnectPlan();

      if (result?.message) {
        toast.success('요금제 해지가 완료되었습니다! 👋', {
          description: `${plan.name} 요금제가 성공적으로 해지되었습니다.`,
        });
        setTimeout(() => {
          navigate('/me');
        }, 2000);
      } else {
        toast.error('요금제 해지에 실패했습니다.', {
          description: '잠시 후 다시 시도해주세요.',
        });
      }
    } catch (error) {
      ...
      <div className="h-full overflow-y-auto pr-3">
        ...
        <div className="space-y-6 py-6">
          ...
          <div className="flex gap-3">
            ...
            {isCurrentPlan && (
              <Button
                onClick={() => handlePlanDisconnect()} // 해지하는 함수 호출
                disabled={isChanging || isDisconnecting}
                className="flex-1 text-red-500 py-3 rounded-xl font-medium transition-all bg-red-100 border-border hover:bg-red-50"
              >
                {isDisconnecting ? (
                  <>
                    <Loader2 className="w-4 h-4 mr-2 animate-spin" />
